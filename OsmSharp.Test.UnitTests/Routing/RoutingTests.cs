...
namespace OsmSharp.Test.Unittests.Routing
{
        ...
        where TEdgeData : IDynamicGraphEdgeData
    {
        ...
        protected void DoTestShortestDefault()
        {
            ...
            data.GetVertexLocation(20, out latitude, out longitude);
            ...
            data.GetVertexLocation(21, out latitude, out longitude);
            ...
            data.GetVertexLocation(16, out latitude, out longitude);
            ...
            data.GetVertexLocation(22, out latitude, out longitude);
            ...
            data.GetVertexLocation(23, out latitude, out longitude);
        ...
        protected void DoTestResolveAllNodes()
        {
            ...
            {
                float latitude, longitude;
                if (data.GetVertexLocation((uint)idx, out latitude, out longitude))
                {
                    GeoCoordinate vertexLocation = new GeoCoordinate(latitude, longitude);
                    RouterPoint point = router.Resolve(Vehicle.Car, vertexLocation);
                    RouterPoint routerPoint = point as RouterPoint;
                    if (idx != routerPoint.Id)
                    { // id's are different, check proxmimity.
                        if (data.GetVertexLocation((uint)routerPoint.Id, out latitude, out longitude))
                        { // vertex was found.
                            Assert.AreEqual(0, new GeoCoordinate(latitude, longitude).Distance(vertexLocation), 0.00001);
                        }
                        else
                        {
                            Assert.Fail("Resolved point {0} is not an existing vertex!",
                                routerPoint.Id);
                        }
                    }
                }
        ...
        protected void DoTestResolveBetweenNodes()
        {
            ...
            for (int idx = 1; idx < data.VertexCount; idx++)
            {
                foreach(KeyValuePair<uint, TEdgeData> arc in data.GetArcs((uint)idx))
                {
                    if (arc.Value.RepresentsNeighbourRelations)
                    {
                        float latitude, longitude;
                        data.GetVertexLocation((uint)idx, out latitude, out longitude);
                        GeoCoordinate vertex1Location = new GeoCoordinate(latitude, longitude);
                        data.GetVertexLocation(arc.Key, out latitude, out longitude);
                        GeoCoordinate vertex2Location = new GeoCoordinate(latitude, longitude);

                        GeoCoordinate center = new GeoCoordinateBox(vertex1Location, vertex2Location).Center;

                        SearchClosestResult result = basicRouter.SearchClosest(data, interpreter, Vehicle.Car, center, delta, null, null);
                        //Assert.IsTrue((result.Vertex1 == idx && result.Vertex2 == arc.Key) ||
                        //    (result.Vertex1 == arc.Key && result.Vertex2 == idx));
                        data.GetVertexLocation((uint)result.Vertex1, out latitude, out longitude);
                        vertex1Location = new GeoCoordinate(latitude, longitude);
                        data.GetVertexLocation((uint)result.Vertex2, out latitude, out longitude);
                        vertex2Location = new GeoCoordinate(latitude, longitude);

                        Assert.AreEqual(0, center.Distance(new GeoCoordinateBox(vertex1Location, vertex2Location).Center), 0.000001);
                    }
                }
            }
        }
